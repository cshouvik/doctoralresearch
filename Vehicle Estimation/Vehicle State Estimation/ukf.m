...
function [xEst,sigma]= ukf(tune,fstate,constS,inpS,instate,inPx,fmeas,measurment,inpM,constM,R,Q,Ts)
...
%             - sigma            : updated covariance of state mean at time k+1
...
% COMPUTATION OF SIGMA POINTS
    r=chol(abs((n+lambda).*Px));

    spts=repmat(mean,1,ns)+[1 r -r];
...
% RESAMPLING OF SIGMA POINTS
    ...
    respts=repmat(meanstate,1,ns)+[1 q -q];
