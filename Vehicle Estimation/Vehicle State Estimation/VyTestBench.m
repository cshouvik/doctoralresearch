...
%VLateral.meas=Vx';                           % Measurement from Sensor
VLateral.meas=AY';  
...
% 
% VLateral.y(:,1) = VyMeas2(VLateral.Ts,VLateral.x(:,1),[Beta],0); % initial onbservation of state
% 
% for k=2:(VLateral.N+1)
%   VLateral.x(:,k) = VyState(VLateral.Ts,VLateral.x(:,k-1),[AY(k-1) phi(k-1) Vx(k-1) YawR(k-1)],VLateral.constS);
%   VLateral.y(:,k) = VyMeas2(VLateral.Ts,VLateral.x(:,1),[Beta],0);
% end
...
for k=2:(VLateral.N),
  ...
  [VLateral.ukf(:,k)] = ukf(VLateral.tune1,'VyState',VLateral.constS,VLateral.inpS,VLateral.x0,VLateral.P0,'VyMeas',VLateral.meas(k),VLateral.inpM,VLateral.constM,VLateral.R,VLateral.Q,VLateral.Ts);
