...
%------------------------- Generate and Load Dataset ---------------------------
    save('10_RunTimeDataset_Sine_accel', 'lf', 'lr', 'l', 'Iz', 'm', 'Ts', 't', 'delta',...
...
%if select == 1   
    ...
    ukfGen.Q = 0.1*eye(5);                        % Process noise variance   repmat([100;100;100000;100000;10000],1,5).
    ukfGen.R = 100*eye(5);                       % Measurement noise variance repmat([1;1;1;1;1],1,5).
    ukfGen.trueS = [Myf'; Myr'; Vy'; YawR'; Beta']; % True State
    ukfGen.x0 = ukfGen.trueS(:,1);                  % initial state 
    ukfGen.P0 = 0.001*eye(5);                      % initial state covariance 
    ...
    ukfGen.tune1 = [0.8 500 0];                    % Tune Parameters
